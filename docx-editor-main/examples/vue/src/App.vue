...
import { de as deLocale, zhCN as zhCNLocale } from '@eigenpal/docx-editor-i18n';
...
const editorLocale = computed(() => {
  if (typeof window === 'undefined') return zhCNLocale;
  const params = new URLSearchParams(window.location.search);
  if (params.get('locale') === 'de') return deLocale;
  if (params.get('locale') === 'en') return undefined;
  return zhCNLocale;
});
